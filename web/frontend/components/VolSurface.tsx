...
/**
 * Plotly 3D vol surface — Plotly is loaded from the CDN on first paint.
 * No npm install needed.  Full drag-rotate, scroll-zoom, shift-drag pan,
 * double-click reset.
 */
export function VolSurface({ forward, baseSigma, rate }: Props) {
  ...
  const atmRidge = useMemo(() => {
    ...
    const idx = Math.max(0, K.findIndex((v, i) =>
    ...
    for (let j = 0; j < data.maturities.length; j++) {
      ...
      zs.push((ivLo + t * (ivHi - ivLo)) * 100);
    }
    return { xs, ys, zs };
  }, [data, forward]);

  // Render / re-render when data, options, or Plotly ready state changes
  useEffect(() => {
    if (!plotlyReady || !data || !divRef.current || !window.Plotly || !atmRidge) return;
    ...
    const traces: any[] = [
      {
        ...
        hovertemplate:
          "<b>K = %{x:.2f}</b><br>" +
          "T = %{y}d<br>" +
          "<b>σ<sub>imp</sub> = %{z:.2f}%</b><extra></extra>",
        name: "IV surface",
      },
      {
        type: "scatter3d",
        ...
        name: "ATM (K = F)",
        hovertemplate: "<b>ATM</b><br>T = %{y}d<br>σ = %{z:.2f}%<extra></extra>",
      },
    ];
    ...
    const layout: any = {
      ...
      hoverlabel: {
        bgcolor: "#0e1117", bordercolor: "#374151",
    ...
    window.Plotly.react(divRef.current, traces, layout, config);
  }, [plotlyReady, data, atmRidge, colorscale, showContours]);

  // Resize on window resize
  useEffect(() => {
    const onResize = () => {
      if (plotlyReady && divRef.current && window.Plotly) {
      ...
      <div className="rounded-lg overflow-hidden border border-ink-600 bg-ink-900">
        {!plotlyReady && (
          <div className="h-[520px] flex items-center justify-center text-sm text-ink-200">
            Loading Plotly 3D engine from CDN…
          </div>
        )}
        <div ref={divRef} style={{ width: "100%", height: plotlyReady ? "540px" : 0 }} />
      </div>
